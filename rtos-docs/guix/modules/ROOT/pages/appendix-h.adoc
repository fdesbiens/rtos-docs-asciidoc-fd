...
APIs. This definition defines the size of this animation control block pool. Defining it as zero removes the animation pool and its related services from the GUIX system component. In that case, the animation support that GUIX Studio generates for a Screen Flow *Animation* action is also compiled out of the specifications file, and any *Animation* action is ignored at runtime.
